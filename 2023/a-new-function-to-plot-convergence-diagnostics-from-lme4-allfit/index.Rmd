...
The plot produced by `plot.fixef.allFit()` by default replaces the colons in interaction effects (e.g., `spin:reg`) with ' &times; ' to facilitate the visibility (this can be overridden by setting `interaction_symbol_x = FALSE`). Yet, it is important to note that any interactions passed to `select_predictors` must have the colon, as that is the symbol present in the `lme4::allFit()` output.
