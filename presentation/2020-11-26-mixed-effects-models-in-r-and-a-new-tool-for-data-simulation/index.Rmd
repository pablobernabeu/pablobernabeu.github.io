...
<div class="mermaid">
graph TD
  A["Variation within factors (participants, trials, items)"] --> B["Linear mixed-effects models: random intercepts and random slopes"]
  B --> C["Fewer false positives and false negatives"]
  B --> D["Fit models in R"]
  D --> E["Maximal approach: all possible random effects"]
  E --> F["Model sometimes fails to converge"]
  F --> G["Progressively lighten the random effects structure"]
  D --> H["Web application for data simulation"]
</div>
```
